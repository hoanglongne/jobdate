...
const ExpCard: React.FC<ExpCardProps> = ({ exp, deleteExperience, index }) => {
    ...
    return (
        <div className='relative border-[2.5px] border-card py-6 px-3 md:px-4 lg:px8 w-full bg-[#FFF9F4] flex flex-col items-center gap-5 md:gap-2 rounded-xl text-card font-medium mb-5 duration-500'>
            <div className='absolute top-3 right-3 cursor-pointer' onClick={() => deleteExperience(index)}>
            ...
            <div className='grid grid-cols-1 md:grid-cols-5 w-full items-center gap-1 md:gap-3'>
                ...
                <p className='text-[#7B7B7B] col-span-4 font-medium'>{exp?.company}</p>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-5 w-full items-center gap-1 md:gap-3'>
                <h1 className='font-bold col-span-1'>Role</h1>
                <p className='text-[#7B7B7B] col-span-4 font-medium'>{exp?.role}</p>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-5 w-full items-center gap-1 md:gap-3'>
                <h1 className='font-bold col-span-1'>Duration</h1>
                <p className='text-[#7B7B7B] col-span-4 font-medium'>{exp?.duration}</p>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-5 w-full items-center gap-1 md:gap-3'>
                <h1 className='font-bold col-span-1'>Desc</h1>
                <p className='text-[#7B7B7B] col-span-4 font-medium'>{exp?.desc}</p>
            </div>
        </div>
    )
}
